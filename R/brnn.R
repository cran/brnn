...
#This function rescales data between -1 and 1
#Note is not the same that 
#mapminmax in matlab, the matlab function will obtain the maximum and minimum for each 
#column and then it will do the calculation
normalize=function(y)
{
  my_min=min(y)
  my_max=max(y)
  if(is.vector(y)) {y=2*(y-my_min)/(my_max-my_min)-1}
  if(is.matrix(y)){
    for(i in 1:ncol(y))
    {
       z=2*(y[,i]-my_min)/(my_max-my_min)-1
       y[,i]=z
    }
  }
  return(y)
...
#This function will obtain a prediction for a BASIC network
#X is the matrix with data
#theta is a list with the network parameters
#neurons is the number of neurons
predictions.nn=function(X,theta,neurons)
{
  n=nrow(X)
  yhat=rep(NA,n)
  for(i in 1:n)
...
#It uses an omp version of the routone predictions.nn to do the work
...
#jacobian=function(y,X,theta,neurons)
#{
#   n=length(y)
#   p=ncol(X)
#   npar=neurons*(1+1+p)
#   J=matrix(NA,nrow=n,ncol=npar)
#   #For every row of the jacobian matrix
#   for(i in 1:n)
#   {
#     #For every neuron
#     x=X[i,]
...
#       z=bias+sum(lambda*x)
#       dtansig=(sech(z))^2
#       J[i,(1+1+p)*(k-1)+1]=tansig(z)                      #Derivate with respect to weight
#       J[i,(1+1+p)*(k-1)+2]=weight*dtansig                 #Derivate with respect to bias
#       #Derivate with respect to the beta's.
#       for(j in 1:length(x))
#       {
#          J[i,(1+1+p)*(k-1)+j+2]=weight*dtansig*x[j]
#       }
#     }
#   }
#   return(-J)
...
brnn=function(y,X,neurons,epochs=1000,mu=0.005,mu_dec=0.1, mu_inc=10,mu_max=1e10,
              ...
              Monte_Carlo=FALSE,tol=1E-6,samples=40)
{
   ...
   #e=y-predictions.nn(X,theta,neurons)
   e=y-predictions.nn.C(vecX,n,p,theta,neurons,cores)
   ...
   while(epoch<=epochs & flag_mu & flag_change_Ed & flag_change_F)
   {
	...
	cat("gradient=",mg,"\n")
	  ...
	  #e_new=y-predictions.nn(X,theta_new,neurons)
          e_new=y-predictions.nn.C(vecX,n,p,theta_new,neurons,cores)
	  Ed=sum(e_new^2)
	  Ew=Ew(theta_new)
   ...
   #answer
   return(list(theta=theta,alpha=alpha,beta=beta,gamma=gamma,Ed=Ed,Ew=Ew,F_history=F_history,reason=reason,epoch=epoch))
}
                       ...
                       min_grad=1e-10,change=0.001,cores=1,verbose=TRUE)
{
   ...
   while(epoch<=epochs & flag_mu & flag_change_Ed & flag_change_F)
   {
	...
	cat("Epoch=",epoch,"\n")
      ...
      #J1=jacobian(y,X1,theta1,neurons1)
      #J2=jacobian(y,X2,theta2,neurons2)
      #J=cbind(J1,J2,-predictions.nn(X1,theta1,neurons1),-predictions.nn(X2,theta2,neurons2))
   ...
   return(list(theta1=theta1,theta2=theta2,c_a=w1,c_d=w2,alpha=alpha,beta=beta,
